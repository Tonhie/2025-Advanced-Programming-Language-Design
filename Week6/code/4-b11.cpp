...
/* -----------------------------------------------------------------------------------
   ...
   ------
----------------------------------------------------------------------------- */
void printChar(char begin, char end) {
	cout << begin;
	if (end == begin)
		return;
	printChar(begin + 1, end);
	cout << begin;
}
/***************************************************************************
  函数名称：
...
***************************************************************************/
void print_tower(char end_ch, int order, int etc)
{
	if (end_ch < 'A')
		return;
	/* 允许按需定义最多一个静态局部变量（也可以不定义） */
	if (order == 1)
		print_tower(end_ch - 1, order, etc + 1);
	cout << setfill(' ') << setw(etc + 1);
	printChar('A', end_ch);
	cout << endl;
	if (order == -1)
		print_tower(end_ch - 1, order, etc + 1);
